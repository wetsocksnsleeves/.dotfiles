---
name: unslop
description: Cut AI tells from any writing. Must always apply. Run together with the no-ai-slop skill.
---
...
Run this alongside the `no-ai-slop` skill, not instead of it: `no-ai-slop` covers voice-preservation, detect-mode, and eval checks this skill doesn't. Apply both pattern lists to the same pass.
...
10. **Mirrored concessive pairs.** "Close enough to read, different enough that the tooling differs", "Simple enough to skim, complex enough to matter", "Familiar enough to guess at, foreign enough to trip on". Two clauses balanced on one axis with a repeated adverb-plus-"enough" frame. The symmetry does the work the facts should be doing, and one half almost always gets overstated to earn the balance. Catch the unstressed variants too: "less about X, more about Y", "not X so much as Y", "X in name, Y in practice". Keep the half that carries information and drop the frame. "Close enough to TypeScript to read, different enough that the tooling is `yarn flow`" becomes "The syntax reads like TypeScript. The checker is `yarn flow`."
